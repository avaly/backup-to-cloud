...
describe('verifier', () => {
	const verify = (awsLSMock, dry) => utils.run(
		[
			'--verbose',
			dry ? '--dry' : '',
			'--aws-ls-mock',
			FIXTURES_DIR + 'verify/' + awsLSMock,
		],
		'verify'
	);
	...
	beforeEach(() => {
		utils.cp(
			FIXTURES_DIR + 'verify/db-test.sqlite',
			config.dbSQLite
		);
	});

	it('OK state', () => {
		return verify('ls-ok.txt')
			.then((output) => {
	...
	it('extra remote file, do nothing', () => {
		return verify('ls-extra-remote.txt')
			.then((output) => {
	...
	it('extra DB file, do nothing', () => {
		return verify('ls-extra-db.txt', true)
			.then((output) => {
				assert.include(
					output,
					'This is a DRY run! No changes/uploads will be made.'
				);
				assert.include(output, 'All remote files are present in the DB!');
