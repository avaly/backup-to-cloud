...
class Transfer {
	...
	next() {
		if (this.sessionSize >= config.maxSessionSize) {
			utils.debug(
				`Transfer.next sessionSize=${this.sessionSize} ` +
		...
		if (this.sessionFailed >= config.maxSessionFailures) {
			utils.debug(
				`Transfer.next sessionFailed=${this.sessionFailed} ` +
		...
		utils.debug('Transfer.next NO_FILES_LEFT');
		return Promise.resolve('NO_FILES_LEFT');
	}

	add(file, remoteFilePath, isDBFile) {
		utils.debug(`Transfer.add: ${file}`);
		if (!isDBFile) {
			this.sessionCount++;
		}
		...
		const upload = (localFile, fileSize) => {
			return this.uploadToS3(localFile,	remoteFile, fileSize).then(
				(stdout) => {
					...
					utils.debug(`Transfer.add success: ${file}`);
					done();
				},
				(err) => {
					utils.log(`Transfer.add error: ${file}`);
	...
	uploadToS3(localFile, remoteFile, fileSize) {
		...
		utils.debug(`Transfer.uploadToS3: ${remoteURL}`);
	...
	encrypt(file) {
		...
		utils.debug(`Transfer.encrypt: ${file} to ${tempFile}`);
	...
	remove(file) {
		utils.debug(`Transfer.remove: ${file}`);
			...
			return this.removeFromS3(remoteFile)
				.then(
					(stdout) => {
						...
						utils.debug(`Transfer.remove success: ${file}`);
						done();
					},
					(err) => {
						utils.log(`Transfer.remove error: ${file}`);
						utils.debug(`Error: ${err}`);
						// Remember failed file to avoid trying it again in this sesssion
						this.skipFiles.push(file);
						this.sessionFailed++;
		...
		utils.debug(
			'Transfer.remove skipping file due to storage class and timestamp'
		);
	...
	removeFromS3(remoteFile) {
		const remoteURL = `s3://${config.s3bucket}${remoteFile}`;
		utils.debug(`Transfer.removeFromS3: ${remoteURL}`);
