...
class Scanner {
	...
	static scanFile(file) {
		...
		return {
			hash: `${utils.remoteFilePath(file)} ${stat.size} ${mtime}`,
			size: stat.size,
		};
	}
	...
	async scanSource(source) {
		...
		const archivesFiles = {};
		lines
			.filter((file) => hasCompressLeaves(file))
			.forEach((file) => {
...
Scanner.findFiles = (dir, args = []) => {
	const result = spawnSync('find', [dir, '-type', 'f', ...args], {
		encoding: 'utf-8',
		maxBuffer: 100 * 1024 * 1024,
	});
	if (result.error || result.status !== 0) {
		throw new Error(
			`find failed for source "${dir}": ${result.stderr || result.error?.message || ''}`,
		);
	}
