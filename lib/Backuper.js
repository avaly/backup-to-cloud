...
class Backuper {
	...
	async next() {
		if (this.sessionFailed >= config.maxSessionFailures) {
			utils.debug(
				`Backuper.next sessionFailed=${this.sessionFailed} maxSessionFailures=${config.maxSessionFailures}`,
			);
		...
		if (!isSessionMaxed) {
			...
			utils.debug(
				`Backuper.next sessionSize=${prettyBytes(this.sessionSize)} maxSessionSize=${prettyBytes(config.maxSessionSize)}`,
			);
		}

		const isRemovalsMaxed =
			config.maxSessionRemovals > 0 && this.sessionRemoved >= config.maxSessionRemovals;
		...
		if (!isRemovalsMaxed) {
			...
			utils.debug(
				`Backuper.next sessionRemoved=${this.sessionRemoved} maxSessionRemovals=${config.maxSessionRemovals}`,
			);
		}
	...
	async add(local, remoteFilePath, isDBFile) {
		...
		const upload = async (localFile, fileSize) => {
			try {
				await this.uploadToS3(localFile, remoteFile, fileSize, hash);
	...
	async remove(remote) {
		...
		let canRemove = true;
		if (remote.size >= config.storageClassIAMinimumSize) {
		...
		const done = async () => {
			...
			await Promise.all([this.db.deleteLocal(file), this.db.deleteRemote(file)]);
		};
