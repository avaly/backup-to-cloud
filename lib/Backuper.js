...
import { debug, execPromise, isDev, isDry, isTest, log, remoteFilePath } from './utils.js';
...
class Backuper {
  ...
  async start() {
    ...
    log('Backuper.start:', `locals=${this.counts.locals} / remotes=${this.counts.remotes}`);
  ...
  async next() {
    const { maxSessionSize, maxSessionRemovals, maxSessionFailures } = config;

    if (this.sessionFailed >= maxSessionFailures) {
      debug(
    ...
    debug(`Backuper.next sessionSize=${prettyBytes(this.sessionSize)}`);
    ...
    debug('Backuper.next NO_FILES_LEFT');
    return 'NO_FILES_LEFT';
  }

  async add(local, remotePathOverride, isDBFile) {
    const backuper = this;
    const { hash, path: file, type } = local;

    debug(`Backuper.add ${type}: ${file}`);

    if (!isDBFile) {
      this.sessionCount++;
    }

    const remoteFile = remotePathOverride || remoteFilePath(file);
  ...
  uploadToS3(localFile, remoteFile, fileSize, hash) {
    ...
    debug(`Backuper.uploadToS3: ${remoteURL} - ${prettyBytes(fileSize)}`);
  ...
  async remove(remote) {
    ...
    debug(`Backuper.remove: ${file}`);

    const remoteFile = remoteFilePath(file);
    ...
    if (canRemove) {
      ...
      try {
        ...
        this.sessionRemoved++;
        debug(`Backuper.remove success: ${file}`);
        done();
      } catch (err) {
        log(`Backuper.remove error: ${file}`);
        debug(`Error: ${err}`);
    ...
    debug('Backuper.remove skipping file due to storage class and timestamp');
  ...
  removeFromS3(remoteFile) {
    ...
    debug(`Backuper.removeFromS3: ${remoteURL}`);
