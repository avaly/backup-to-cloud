...
class DB {
	...
	initializeTables() {
		...
		if (!db.prepare(Q_TABLE_EXISTS).get({ name: 'settings' })) {
			db.prepare(`
		...
		if (!db.prepare(Q_TABLE_EXISTS).get({ name: 'locals' })) {
			db.prepare(`
				CREATE TABLE locals(
		...
		if (!db.prepare(Q_TABLE_EXISTS).get({ name: 'remotes' })) {
			db.prepare(`
