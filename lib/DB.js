...
const DB_FILE = runtime.resolveAppPath(config.dbSQLite);
...
class DB {
  ...
  async getSetting(name, defaultValue) {
    try {
      const { value } = await this.db.get('SELECT value FROM settings WHERE name=?', name);
      return value;
    } catch {
      return defaultValue;
    }
  ...
  async getCounts() {
    try {
      const { cnt: localsCount } = await this.db.get('SELECT COUNT(*) AS cnt FROM locals');
      const { cnt: remotesCount } = await this.db.get('SELECT COUNT(*) AS cnt FROM remotes');
