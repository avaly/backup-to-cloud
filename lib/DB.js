...
import { getBundledBetterSqlite3BindingPath } from './sea.js';
import { DELETED, debug, execSync, isDev, isDry, isTest, mkdir } from './utils.js';

const DB_FILE = path.resolve(process.cwd(), config.dbSQLite);
...
class DB {
  constructor(file = DB_FILE) {
    ...
    this.file = file + (isDry() ? '.dry' : '');

    const dir = path.dirname(file);
    mkdir(dir);

    if (isDry()) {
      execSync('cp', [file, this.file]);
    }
  }

  initialize() {
    const nativeBinding = getBundledBetterSqlite3BindingPath();

    this.db = new Database(this.file, {
      ...(nativeBinding ? { nativeBinding } : {}),
      verbose:
        process.env.DB_DEBUG && (isDev() || isTest()) ? (sql) => debug('db.trace', sql) : undefined,
    });
