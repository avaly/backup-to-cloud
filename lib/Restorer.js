...
class Restorer {
	...
	fetchRemoteDB() {
		...
		const dbRemoteFile = path.basename(config.db);
		...
		return utils.execPromise(AWS, args)
			.then((output) => {
				...
				this.db = JSON.parse(
					fs.readFileSync(dbLocalFile, 'utf-8')
				);
			});
	}

	filter() {
		this.synced = this.db.synced;
		this.syncedFiles = Object.keys(this.synced).sort();
		this.restoreFiles = this.syncedFiles.filter(
			(file) => utils.remoteFilePath(file).indexOf(this.remotePrefix) === 0
		);
