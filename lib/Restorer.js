...
class Restorer {
	...
	filter() {
		...
		const count = this.restoreQueue.length;
		const totalSize = this.restoreQueue.reduce((accumulated, item) => accumulated + item.size, 0);
		...
		if (utils.hasFlag('dry') || utils.hasFlag('yes')) {
			return this.next();
		}
