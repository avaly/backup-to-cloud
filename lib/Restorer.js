...
class Restorer {
	async start(remotePrefix, localPath) {
		...
		if (utils.hasFlag('max-size')) {
			...
			if (Number.isNaN(parseInt(maxSizeOption, 10))) {
				utils.log(
					'Restorer.filter:',
					`invalid --max-size value "${maxSizeOption}". Please provide a numeric value.`,
				);
	...
	filter() {
		const maxSize = utils.hasFlag('max-size') ? parseInt(utils.getOption('max-size'), 10) : null;
		...
		let queue = this.data.remotes.filter(
			remote =>
				...
				(maxSize ? remote.size <= maxSize : true),
		);
		...
		// that case explicitly and return a clear failure result / message.
		if (isTest && count === 0) {
			utils.log('Restorer.filter: no matching files found - aborting without running test');
			return Promise.resolve('NO_MATCHING_FILES');
		}
