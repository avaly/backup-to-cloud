...
class Restorer {
	...
	filter() {
		this.restoreQueue = this.data.remotes
			.filter(
				(remote) => utils.remoteFilePath(remote.path)
					.indexOf(this.remotePrefix) === 0
			);

		utils.log(
			`Restorer.filter: ${this.restoreQueue.length} matching files in DB`
		);
